...
2.  **Acceptance Criteria**:
    1.  While the system is running, it shall provide a web-based admin dashboard.
    2.  The admin dashboard shall be built using Next.js and TypeScript.
    3.  The user interface shall be implemented using ShadCN UI components and Tailwind CSS.
    4.  The admin dashboard shall be responsive and accessible on modern web browsers, including desktop and mobile devices.
...
2.  **Acceptance Criteria**:
    1.  The system shall have a dedicated API service layer for communicating with the backend.
    2.  The API service layer shall use Axios for HTTP requests.
    3.  The system shall handle API errors gracefully and provide feedback to the user.
    4.  The system shall use environment variables to configure the backend API URL.

### 3. User Management

1.  **User Story**: As an administrator, I want to manage user accounts so that I can maintain a healthy and safe user community.
2.  **Acceptance Criteria**:
    1.  The system shall provide an interface to list all users with pagination and sorting.
    2.  The system shall allow administrators to search and filter the user list.
    ...
    6.  The system shall support bulk operations on users (e.g., activate, deactivate, delete).
...
2.  **Acceptance Criteria**:
    ...
    3.  The system shall allow administrators to preview media content (images and videos).
    4.  The system shall allow administrators to view media metadata.
    5.  The system shall allow administrators to delete media content.
...
2.  **Acceptance Criteria**:
    1.  The system shall have a dashboard overview page.
    2.  The dashboard shall display key metrics, such as total users, active users, and total media files.
    3.  The dashboard shall display charts for user growth and media uploads.
    4.  The dashboard shall have a feed of recent system activity.
...
2.  **Acceptance Criteria**:
    1.  The system shall require administrators to log in to access the dashboard.
    2.  The system shall have protected routes that are only accessible to authenticated users.
...
2.  **Acceptance Criteria**:
    1.  The system shall use TanStack Query for server state management.
    2.  The system shall implement optimistic updates for a better user experience.
    3.  The system shall use code splitting and lazy loading to optimize performance.
